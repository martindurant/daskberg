...
def convert(value, type):
    ...
    if type == "int":
        ...
        # might be better to carry int and scale instead of suffering float errors
        scale = int(type.rsplit(",", 1)[-1].rstrip(")"))
        return int.from_bytes(value, "little") / 10**scale
    elif type == "date":
        return np.array(int.from_bytes(value, "little") * DAYS_TO_MILLIS, dtype="M8[ms]")
    else:
        raise TypeError
...
def transform(val, trans):
    ...
    raise NotImplementedError(f"Transform for value <{val}> with transformer <{trans}>")
